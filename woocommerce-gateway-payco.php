    ...
    function init_epayco_woocommerce()
    {
        ...
        class WC_ePayco extends WC_Payment_Gateway
        {
            ...
            public function receipt_page($order_id)
            {
                ...
                $pos = strpos($order->get_total(), '.');
                $rest = substr($order->get_total(), $pos);
                $rounnd = str_replace('.', '',  $rest);
                ...
                foreach ($order->get_items() as $item_key => $item ){
                    ...
                    $dokan_admin_percentage = 'SELECT meta_value FROM '.$table_name_ .' WHERE user_id = '. intval($vendor_id).' AND meta_key = "dokan_admin_percentage"';
                    $dokan_admin_percentage_type = 'SELECT meta_value FROM '.$table_name_ .' WHERE user_id = '. intval($vendor_id).' AND meta_key = "dokan_admin_percentage_type"';
                    $dokan_nickname = 'SELECT meta_value FROM '.$table_name_ .' WHERE user_id = '. intval($vendor_id).' AND meta_key = "nickname"';
                    $admin_percentage = $wpdb->get_results($dokan_admin_percentage, OBJECT);
                    $admin_percentage_type = $wpdb->get_results($dokan_admin_percentage_type, OBJECT);
                    $dokan_nickname_ = $wpdb->get_results($dokan_nickname, OBJECT);

                    if($admin_percentage_type[0]->meta_value == "flat"){
                        $p_split_type_ = '01';
                    }else{
                        $p_split_type_ = '02';
                    }
                    $other = array(
                        "role"=>$vendor->roles[0],
                        "tipo"=> $p_split_type_,
                        "valor"=> $admin_percentage[0]->meta_value,
                        "id_vendor"=> intval($vendor_id),
                        "vendor_email"=>$vendor->user_email,
                        "vendor_nicename"=>$vendor->user_nicename,
                        "vendor_nickname"=>$dokan_nickname_[0]->meta_value,
                ...
                foreach($items as $item => $value) {
                    ...
                    $total=floatval($subtotal+$regulart_product_tax);
                    foreach ($vendorsArray as $key) {
                        //buscar productos del administrador y configurar su comision
                        if ($key['role'] != 'administrator'){
                            if( $value['product_id'] == $key['vendor_productoId'] ){

                                if($key['tipo']=='01'){
                                    $valor = (intval($key['valor'])*$value['quantity']);
                                }else{
                                    $valor = (($total*intval($key['valor']))/100);
                                }

                                $other = array(
                                    "id"=>$key['vendor_nickname'],
                                    "iva"=>round($regulart_product_tax,strlen( $rounnd)),
                                    "base_iva"=>round($subtotal,strlen( $rounnd)),
                                    "total" => round($total,strlen( $rounnd)),
                                    "fee"=>round($valor,strlen( $rounnd)),
                                );
                                array_push($split_receivers, $other );
                            }
                        }
                    }
                }
                var_dump(json_encode($split_receivers));
                die();
